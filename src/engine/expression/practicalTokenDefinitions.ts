...
export const PRACTICAL_EXPRESSION_TOKEN_DEFINITIONS: readonly ExpressionTokenDefinition[] = [
  ...
  chineseCalendar('festival.today', 'cn2', 'Festival Today', '今日优先节日', '中秋节', 'string', true),
  chineseCalendar('festival.solar', 'cn2.1', 'Gregorian Festival', '今日公历节日', '国庆节', 'string', true),
  chineseCalendar('festival.lunar', 'cn2.2', 'Lunar Festival', '今日农历节日', '中秋节', 'string', true),
  chineseCalendar('festival.next', 'cn2.3', 'Next Festival', '下一个节日', '中秋节', 'string', true),
  chineseCalendar('festival.days', 'cn2.4', 'Days to Festival', '距下一个节日天数', 12, 'number', true),
  chineseCalendar('solarTerm.today', 'cn2.5', 'Solar Term Today', '今日节气', '立秋', 'string', true),
  chineseCalendar('solarTerm.next', 'cn2.6', 'Next Solar Term', '下一个节气', '处暑', 'string', true),
  chineseCalendar('solarTerm.days', 'cn2.7', 'Days to Solar Term', '距下一个节气天数', 7, 'number', true),
