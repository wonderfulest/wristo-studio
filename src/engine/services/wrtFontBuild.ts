...
import { completeIconBuild, requiredIconGlyphs } from './wrtIconCoverage'
...
export async function buildMissingWrtFonts(zip: JSZip, fonts: WrtSourceFont[], onProgress?: (progress: WrtFontBuildProgress) => void, config?: unknown) {
  // Preserve existing bitmap fonts, including subsets. Missing icon glyphs must
  // not trigger an expensive rebuild or prevent saving/reopening a design.
  ...
  try {
    for (const [index, font] of missing.entries()) {
      ...
      const isIcon = font.metadata?.type === 'icon_font'
        || (config as any)?.elements?.some((element: any) => element.eleType === 'icon'
          && element.iconDisplayType !== 'amoled' && (element.fontFamily || element.iconFont) === font.slug)
      ...
      if (isIcon) await completeIconBuild(generated,
        font.slug === 'wristo-icon' && requiredIconGlyphs(config, font.slug).has(104)
      ...
      const metadata = { ...font.metadata, slug: font.slug, fullName: font.metadata?.fullName || font.slug,
        type: font.metadata?.type || (isIcon ? 'icon_font' : 'text_font'), language: font.metadata?.language || 'en',
      ...
      // Keep existing assets and nonstandard sizes instead of silently dropping them.
