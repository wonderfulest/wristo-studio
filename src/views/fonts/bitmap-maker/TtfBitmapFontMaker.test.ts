...
import BitmapFontMaker from './TtfBitmapFontMaker.vue'
...
async function upload(wrapper: ReturnType<typeof mountMaker>, name = 'Precision Sans.ttf') {
  ...
  await vi.waitFor(() => expect((wrapper.vm as any).metadata.slug).not.toBe(''))
  await nextTick()
  return file
}
...
describe('BitmapFontMaker', () => {
  beforeEach(() => {
    ...
    Object.defineProperty(document, 'fonts', {
      configurable: true,
      value: {
        add: mocks.fontFaceAdd,
        delete: mocks.fontFaceDelete,
      },
    })
  ...
  it('keeps slug, style tags, and search keywords in sync with the recipe while preserving comma-separated manual values', async () => {
    ...
    expect(vm.searchKeywordsInput).toBe('precision sans, number, time, bitmap, regular, fill')
    expect((wrapper.get('[data-test="style-tags-input"]').element as HTMLInputElement).value).toBe('regular, fill')
    expect((wrapper.get('[data-test="search-keywords-input"]').element as HTMLInputElement).value).toBe('precision sans, number, time, bitmap, regular, fill')

    vm.styleTagsInput = 'sport'
  ...
  it('publishes changed edit recipes as new fonts and restores overwrite mode when reverted', async () => {
    mocks.route.query = { fontId: '42', fontType: 'text_font_zh' }
    mocks.getFont.mockResolvedValue({
      code: 0,
      data: {
        id: 42,
        fullName: '我的中文字体',
        slug: 'my-chinese-font',
        type: 'text_font_zh',
        language: 'zh',
        styleTags: 'rounded,regular',
        searchKeywords: '中文,圆润',
        bitmapRecipe: JSON.stringify({
          schemaVersion: 1,
          rendererVersion: '1',
          fontWeight: 600,
          italicAngle: -4,
        ...
        ttfFile: { name: 'source.ttf', url: 'https://assets.example/source.ttf' },
      },
    })
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, blob: () => Promise.resolve(new Blob([new Uint8Array([1, 2, 3])], { type: 'font/ttf' })) }))

    const wrapper = mountMaker()
    const vm = wrapper.vm as any
    await vi.waitFor(() => expect(vm.sourceFile?.name).toBe('source.ttf'))
    ...
    await vm.buildPackage()
    await vm.publishPackage()
    expect(mocks.publish).toHaveBeenCalledWith(expect.objectContaining({ fontId: 42, overwrite: true }))
    expect(mocks.publish.mock.calls.at(-1)?.[0].metadata.slug).toBe('my-chinese-font')

    vm.recipe.fontWeight = 700
    await vi.waitFor(() => expect(vm.metadata.slug).not.toBe('my-chinese-font'))
    await vm.buildPackage()
    await vm.publishPackage()
    expect(mocks.publish.mock.calls.at(-1)?.[0]).toEqual(expect.objectContaining({ fontId: undefined, overwrite: false }))
    expect(mocks.publish.mock.calls.at(-1)?.[0].metadata.slug).not.toBe('my-chinese-font')

    vm.recipe.fontWeight = 600
    await vi.waitFor(() => expect(vm.metadata.slug).toBe('my-chinese-font'))
    await vm.buildPackage()
    await vm.publishPackage()
    expect(mocks.publish.mock.calls.at(-1)?.[0]).toEqual(expect.objectContaining({ fontId: 42, overwrite: true }))
  })
  ...
  it('requires redistribution attestation only for publish and enforces tag contract bounds', async () => {
    ...
    await nextTick()
    expect(vm.normalizedStyleTags).toEqual(['regular', 'fill', 'x'.repeat(32), 'sport'])
