      ...
      <section class="control-stack">
        <article class="panel source-panel">
          ...
          <div class="segmented" role="radiogroup" :aria-label="t('bitmapMaker.fontType')">
            <label><input v-model="fontType" type="radio" value="number_font" :disabled="actionsLocked || editingFontId != null" /><span>{{ t('bitmapMaker.numbers') }}</span></label>
            <label><input v-model="fontType" type="radio" value="text_font" :disabled="actionsLocked || editingFontId != null" /><span>{{ t('bitmapMaker.englishText') }}</span></label>
            <label><input v-model="fontType" type="radio" value="text_font_zh" :disabled="actionsLocked || editingFontId != null" /><span>{{ t('font.languageChinese') }}</span></label>
          </div>
        </article>
        ...
        <article class="panel metadata-panel">
          ...
          <label class="field-label">{{ t('bitmapMaker.slug') }}<input ref="slugInput" v-model.trim="metadata.slug" data-test="slug-input" type="text" readonly :disabled="actionsLocked" :class="{ invalid: slugConflict }" /></label>
...
const previewSample = computed(() => fontType.value === 'number_font' ? '10:09' : fontType.value === 'text_font_zh' ? '周三 24' : 'WED 24')
const previewTextStyle = computed(() => {
  ...
  const syntheticWeightStroke = recipe.outlineMode === 'fill' && recipe.fontWeight > sourceWeight
    ? ((recipe.fontWeight - sourceWeight) / 500) * 44 * 0.04
    : 0
  const strokeWidth = hasOutline ? recipe.outlineWidthEm * 44 : syntheticWeightStroke
  return {
...
async function loadEditingFont() {
  ...
  try {
    ...
    if (!font || !['number_font', 'text_font', 'text_font_zh'].includes(font.type)) {
...
@media(max-width:900px){.bitmap-workbench{padding:0 16px 16px}.workbench-grid{grid-template-columns:1fr}.preview-stage{min-height:600px}.stage-rail span{padding:10px;font-size:10px}.stage-rail span b{display:none}}@media(max-width:560px){.stage-rail{grid-template-columns:1fr 1fr}.stage-rail span:nth-child(2){border-right:0}.stage-rail span:nth-child(-n+2){border-bottom:1px solid var(--studio-border)}.workbench-grid{display:block}.control-stack{margin-bottom:14px}.preview-stage{min-height:540px}.atlas-frame{min-height:280px}.device-preview{flex-direction:column;gap:14px}.watch-ring{width:160px;height:160px;border-width:10px}.watch-ring span{font-size:34px}.action-bar{display:grid;grid-template-columns:1fr 1fr}.button.publish{grid-column:1/-1}}
</style>
