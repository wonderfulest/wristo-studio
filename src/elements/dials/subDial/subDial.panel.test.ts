...
describe('subDial panel content controls', () => {
  it('uses the mode-safe Dial Property selector', () => {
    expect(source).toContain('DialPropertyField')
    expect(source).toContain('model.dialProperty')
  ...
  it('defines every panel translation in English and Chinese', () => {
    ...
    const dynamicKeys = ['goal','range','icon','label','value','unit','goalRange','percentage','mip','amoled','classic','compact','goalFocus','font','prefix','suffix'].map(key => `subDial.${key}`)
