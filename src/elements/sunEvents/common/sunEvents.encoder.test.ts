...
import { resolveSunEventIndicatorSource, scaleArcSunEventsConfig } from './sunEvents.geometry'
...
describe('Sun Events encoders', () => {
  ...
  it('uses the bundled triangle SVG as the default current-time indicator', () => {
    for (const schema of [arcSunEventsSchema, lineSunEventsSchema]) {
      ...
      expect(decodeURIComponent(source)).toContain('fill="white"')
    }
  })
