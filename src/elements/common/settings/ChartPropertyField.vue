<template>
  ...
  >
    ...
    >
      <el-option
        v-for="[key, prop] in chartOptions"
        :key="key"
...
const { t } = useI18n()
const chartOptions = computed(() => Object.entries(propertiesStore.allProperties).filter(([_, p]) => p.type === 'chart'))
const resolvedLabel = computed(() => props.label || t('property.chartSelect'))
