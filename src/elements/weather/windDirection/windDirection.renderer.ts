...
async function resolveSourceSize(url: string, fallbackWidth: number, fallbackHeight: number): Promise<{ width: number; height: number }> {
  ...
  if (svgTextFromData) {
    ...
    console.info('[WindDirection][resolveSourceSize:data-url]', { parsed, fallback, result })
    return result
  }

  if (!/\.svg([?#].*)?$/i.test(url)) {
    console.info('[WindDirection][resolveSourceSize:non-svg-fallback]', { url, fallback })
    return fallback
  }
  ...
  try {
    ...
    const result = parsed ?? fallback
    console.info('[WindDirection][resolveSourceSize:remote-svg]', { url, parsed, fallback, result })
    return result
  } catch (err) {
    console.warn('[WindDirection][resolveSourceSize:fetch-failed]', { url, fallback, err })
    return fallback
  }
}
...
function commitManualGroupScale(group: FabricGroup): void {
  ...
  const nextW = Math.max(
    1,
    Number((group as unknown as { getScaledWidth?: () => number }).getScaledWidth?.() ?? group.width ?? 1),
  )
  const nextH = Math.max(
    1,
    Number((group as unknown as { getScaledHeight?: () => number }).getScaledHeight?.() ?? group.height ?? 1),
  )
...
export async function createWindDirection(config: WindDirectionElementConfig): Promise<FabricElement> {
  ...
  try {
    ...
    const sourceSize = await resolveSourceSize(imageUrl, imgEl.naturalWidth || 1, imgEl.naturalHeight || 1)
    ...
    console.warn('[WindDirection] createWindDirection load image failed, fallback to empty group', err)
    return await createEmptyGroup()
  }
}
...
export async function updateWindDirection(
  ...
  let imageObj = children.find((c) => (c as unknown as { role?: string }).role === 'image') as unknown as
    | FabricImage
    | undefined
  ...
  if (nextAssetId && (isAssetChanged || !isSvgLikeUrl(nextUrl))) {
    try {
      ...
      console.warn('[WindDirectionRenderer] updateWindDirection resolve source url by assetId failed', nextAssetId, err)
    }
  }
  ...
  const applyImageToGroup = (imgObj: FabricImage, sourceWidth?: number, sourceHeight?: number) => {
    ...
    const iw = Math.max(
      1,
      Number(sourceWidth ?? fallbackSourceW ?? (imgObj as unknown as { width?: number }).width ?? 1),
    )
    const ih = Math.max(
      1,
      Number(sourceHeight ?? fallbackSourceH ?? (imgObj as unknown as { height?: number }).height ?? 1),
    )
  ...
  const shouldReloadImage = Boolean(
    nextUrl && (nextUrl !== storedUrl || config.color !== undefined || !imageObj),
  )
  ...
  if (shouldReloadImage && nextUrl) {
    try {
      ...
      if (!imageObj) {
        ...
        try {
          ;(imageObj as unknown as { setElement?: (el: HTMLImageElement) => void }).setElement?.(
      ...
      console.error('[WindDirectionRenderer] updateWindDirection reload error', err)
    }
  } else if (hasExplicitSize) {
    const targetW = Number(config.width ?? currentStoredW ?? 60)
    const targetH = Number(config.height ?? currentStoredH ?? 60)
    await setWindDirectionImageSize(element, targetW, targetH)
  }
...
export async function setWindDirectionImageSize(
  element: FabricElement,
  width: number,
  height: number,
): Promise<void> {
  ...
  attachManualScaleSync(obj)
  const imageObj = obj.getObjects().find((c) => (c as unknown as { role?: string }).role === 'image') as unknown as
    | FabricImage
    | undefined
  ...
  if (imageObj) {
    const sourceW =
      (obj as unknown as { windSourceWidth?: number }).windSourceWidth ??
      (imageObj as unknown as { width?: number }).width ??
      1
    const sourceH =
      (obj as unknown as { windSourceHeight?: number }).windSourceHeight ??
...
export async function setWindDirectionDegree(element: FabricElement, degree: number): Promise<void> {
  const canvasStore = useCanvasStore()
  const canvas = canvasStore.canvas
  if (!canvas) return
