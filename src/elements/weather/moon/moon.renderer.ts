...
export async function createMoon(config: MoonElementConfig): Promise<FabricElement> {
  ...
  if (!canvas) {
    throw new Error('Canvas is not initialized, cannot add moon element')
  }
  ...
  return await new Promise<FabricElement>((resolve) => {
    ...
    let resolved = false
    setTimeout(() => {
      if (!resolved) console.warn('[MoonRenderer] createMoon image load not resolved yet', { imgUrl })
    }, 2000)
...
export function setMoonImageSize(element: FabricElement, width: number, height: number): void {
  const canvasStore = useCanvasStore()
  const canvas = canvasStore.canvas
  if (!canvas) return
  ...
  const imageObj = children.find((c) => (c as unknown as { role?: string }).role === 'image') as unknown as
    | FabricImage
    | undefined
  ...
  if (imageObj) {
    ...
    console.warn('[MoonRenderer] setMoonImageSize no image child found in group; updating metadata only')
  }
...
export function updateMoon(element: FabricElement, config: Partial<MoonElementConfig>): void {
  const canvasStore = useCanvasStore()
  const canvas = canvasStore.canvas
  if (!canvas) return
  ...
  const prevPos = shouldLockPosition
    ? {
        left: (obj as unknown as { left?: number }).left,
        top: (obj as unknown as { top?: number }).top,
      }
    : null
  ...
  if (!imageObj && newUrl) {
    let resolved = false
    setTimeout(() => {
      if (!resolved) console.warn('[MoonRenderer] updateMoon image load not resolved yet', { newUrl })
    }, 2000)
    ...
    loadHtmlImage(String(newUrl))
      .then((imgEl) => {
        ...
        obj.setCoords()
        try {
          ;(obj as unknown as { bringToFront?: () => void }).bringToFront?.()
        } catch {}
        canvas.setActiveObject(obj as unknown as FabricObject)
        canvas.requestRenderAll()
      })
      .catch((err) => {
        console.error('[MoonRenderer] updateMoon image load error', { newUrl, err })
      })
  } else if (imageObj) {
    if (newUrl && newUrl !== currentUrl) {
      ...
      setTimeout(() => {
        if (!resolved)
          console.warn('[MoonRenderer] updateMoon replace image load not resolved yet', { to: config.imageUrl })
      }, 2000)
      ...
      loadHtmlImage(String(newUrl))
        ...
        .catch((err) => {
          console.error('[MoonRenderer] updateMoon replace image load error', { to: config.imageUrl, err })
        })
    } else {
      const targetW = Math.max(
        1,
        Number(newW ?? (obj as unknown as { width?: number }).width),
      )
      const targetH = Math.max(
        1,
        Number(newH ?? (obj as unknown as { height?: number }).height),
      )
      setMoonImageSize(element, targetW, targetH)
    }
  }
