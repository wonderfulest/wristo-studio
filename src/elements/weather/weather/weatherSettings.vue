...
const onWidthChange = (v: number) => {
  const width = normalizeSize(v)
  applyUpdate({ width, height: width })

  if (!props.applyPatch && props.element) {
    weatherStore.setImageSize(props.element, width, width)
  }
}

const onHeightChange = (v: number) => {
  const height = normalizeSize(v)
  applyUpdate({ width: height, height })

  if (!props.applyPatch && props.element) {
    weatherStore.setImageSize(props.element, height, height)
  }
}
