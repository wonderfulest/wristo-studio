...
export function updateBattery(element: FabricElement, patch: Partial<BatteryElementConfig> = {}): void {
  ...
  batteryHead.set({
    width: next.headWidth,
    height: next.headHeight,
    fill: next.headFill,
    rx: next.headRx,
    ry: next.headRy,
    originX: 'left',
    originY: 'center',
    left: next.width / 2 + nextHeadGap,
  })
  ...
  batteryLevel.set({
    width: (next.width - padding * 2) * next.level,
    height: next.height - padding * 2,
    fill: getLevelColor(next.level, nextLow, nextMedium, nextHigh),
    originX: 'left',
    originY: 'center',
    left: -next.width / 2 + padding,
  })
