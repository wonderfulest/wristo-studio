  ...
  actions: {
    ...
    async addElement(config: HandElementConfig) {
      ...
      const options = {
        ...
        left: config.left ?? this.designStore.watchSize / 2,
        top: config.top ?? this.designStore.watchSize / 2,
        imageUrl: imageUrl,
      ...
      if (iw > 0 && ih > 0) {
        const scale = this.designStore.watchSize / Math.max(iw, ih)
      ...
      this.canvas?.setActiveObject(img as unknown as FabricObject)
    ...
    async updateHandSVG(element: FabricElement, config: HandElementConfig) {
      ...
      if (config.imageUrl && config.imageUrl !== hand.imageUrl) {
        ...
        this.canvas.remove(hand as unknown as FabricObject)
        ...
        this.canvas.add(hand as unknown as FabricObject)
      }
