...
export const useSecondHandStore = defineStore('secondHandElement', {
  ...
  actions: {
    ...
    async addElement(config: HandElementConfig) {
      ...
      if (iw > 0 && ih > 0) {
        const scale = this.designStore.designSpec.width / Math.max(iw, ih)
    ...
    updateAngle(element: any, angle: number) {
      if (!this.canvas) return
      const secondHand: any = this.canvas.getObjects().find((obj: any) => obj.id === element.id)
      if (!secondHand) return
      this.rotateHand(secondHand, angle)
    },
    updateTime(time?: Date) {
      if (!this.canvas) return
      const secondHand: any = this.canvas.getObjects().find((obj: any) => obj.eleType === 'secondHand')
      if (!secondHand) return
