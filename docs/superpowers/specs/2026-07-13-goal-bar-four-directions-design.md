...
- 切换到 `horizontal` 时将方向重置为 `leftToRight`，并交换 `width` 与 `height`。
- 切换到 `vertical` 时将方向重置为 `bottomToTop`，并交换 `width` 与 `height`。
- 不记忆另一个 orientation 下的历史方向。
- 重复选择当前 orientation 时不更新、不交换尺寸。
...
上述三组设置仅位于右侧设置面板，不在画布上增加快捷控件。现有下拉框改为紧凑的 segmented icon buttons：

- Orientation：水平进度条图标、垂直进度条图标。
- Shape：矩形图标、自定义多边形图标。
- Progress Direction：水平方向显示向右/向左箭头，垂直方向显示向上/向下箭头。

选中项使用主题色边框和浅色背景。每个按钮必须提供英文 tooltip、`aria-label`、键盘焦点和可识别的选中状态。图标按钮只替换选择器表现形式，不改变 Orientation 尺寸交换、Shape 多边形创建/编辑或 Progress Direction 绘制语义。

这是面向终端用户的设置文案，因此保持英文。切换 Orientation 时原地交换元素的 `width` 和 `height`，保持 `left`、`top` 与元素中心位置不变，并重置为该轴向的默认进度方向。Orientation、Progress Direction、width 和 height 必须在同一次更新中提交，使撤销/重做只产生一个历史步骤。自定义多边形继续使用归一化顶点，因此无需改写点数据。
