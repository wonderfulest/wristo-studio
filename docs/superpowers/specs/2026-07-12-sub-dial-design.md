...
Studio 新增独立的 `subDial` 元素，用一根指针把单个目标动态变量的完成进度映射到仪表盘角度。首版支持百分比范围与自定义范围，默认采用 240° 仪表式布局，并支持矢量指针和自定义图片指针。

首版只支持单目标变量、单指针、单数值标签。双指针、彩色数值区间和内部子元素独立编辑不在本期范围内。
...
1. **Goal**：目标动态变量、范围模式、最小值、最大值、预览值、越界行为。
...
`subDial` 只支持 `goalProperty`，不支持 `dataProperty`。目标动态变量沿用 `goalBar` 和 `goalArc` 的字段体系，不为 `subDial` 新建独立数据源。新建元素时自动分配未使用的目标动态变量；属性面板通过 `GoalPropertyField` 完成选择和重绑。

## 7. Studio 数据流

`subDial` 通过元素注册体系接入创建、更新、编码、解码和设置面板。配置更新应通过现有 element manager 路径同步 Fabric 对象、元素 store、图层 store、历史记录和序列化配置。

`goalProperty` 同时保存在 live Fabric 对象和 `__element.config` 中。重新绑定时必须同步两处；encoder 优先读取 live Fabric 对象的 `goalProperty`，避免导出陈旧配置。类型、schema、panel、renderer、encoder 和 decoder 均不得继续保留 `dataProperty` 别名或旧字段兼容逻辑。
...
Studio 导出扁平化的运行时配置，包含位置、半径、`goalProperty`、范围、角度、刻度、指针资源 ID、图片轴心、缩放和偏移。脚手架只读取并输出 `goalProperty`，不接受或迁移旧 `dataProperty`。

共享运行时类 `SubDial.mc` 放在 `wristo-apps/SuperBarrel`，负责：

- 从 `DataFetcher` 获取目标当前值和目标值，并按 `goalBar`、`goalArc` 相同的表达式计算完成进度：`goalProperty_fetch[DATA_LOGIC_VALUE] / goalProperty_fetch[DATA_GOAL]`。
- 归一化数值并计算指针角度。
- 绘制背景、刻度、数值和单位。
...
设备模板将上述目标完成进度传给 `SubDial.draw`。运行时把 `0–1` 完成进度换算到现有百分比刻度后再执行范围归一化，因此 Percentage 模式仍显示和配置为 `0–100`；目标值缺失、为零或结果无效时，不绘制指针和动态数值。
...
- 未绑定 `goalProperty`：阻止生成无效的设备端变量引用。
- 目标值缺失或为零：保留静态表盘，隐藏指针和动态数值。
